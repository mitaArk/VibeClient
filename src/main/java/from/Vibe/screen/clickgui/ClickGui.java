...
public class ClickGui extends Screen implements Wrapper {
    ...
    public ClickGui() {
        super(Text.of("Vibe-clickgui"));
        for (Category category : Vibe.getInstance().getModuleManager().getCategories()) {
    ...
    public void render(DrawContext context, int mouseX, int mouseY, float delta) {
        if (close && openAnimation.getValue() <= 0.01f) {
            Vibe.getInstance().getModuleManager().getModule(UI.class).setToggled(false);
            super.close();
            return;
        }
