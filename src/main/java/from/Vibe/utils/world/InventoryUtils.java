package from.Vibe.utils.world;

import from.Vibe.Vibe;
import from.Vibe.api.mixins.accessors.IClientPlayerInteractionManager;
import from.Vibe.modules.impl.movement.GuiMove;
...
public class InventoryUtils implements Wrapper {
    ...
    public void switchSlot(Switch mode, int slot, int previousSlot) {
        if (slot == -1 || previousSlot == -1 || slot == Vibe.getInstance().getServerManager().getServerSlot()) return;
    ...
    public void bypassSwap(int slot, int targetSlot) {
        ...
        if (Vibe.getInstance().getModuleManager().getModule(GuiMove.class).funtime.getValue()) {
           	Vibe.getInstance().getModuleManager().getModule(GuiMove.class).setTicks(8);
           	
        	new Thread(() -> {
                try {
                	if (Vibe.getInstance().getModuleManager().getModule(GuiMove.class).funtime.getValue()) Thread.sleep(Vibe.getInstance().getModuleManager().getModule(GuiMove.class).getTicks() * 50L);
