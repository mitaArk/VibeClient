package from.Vibe.utils.combat;
...
public class IdealHitUtils implements Wrapper {
    ...
    public boolean canAIFall() {
        return ((getBlock(0, 3, 0) == Blocks.AIR && getBlock(0, 2, 0) == Blocks.AIR && getBlock(0, 1, 0) == Blocks.AIR)
                || Vibe.getInstance().getServerManager().getFallDistance() < (getBlock(0, 2, 0) != Blocks.AIR ? 0.08f : 0.6f)
                || Vibe.getInstance().getServerManager().getFallDistance() > 1.2f);
    }
    ...
    public boolean canCritical(LivingEntity target) {
        ...
        return (!mc.player.isOnGround() && Vibe.getInstance().getServerManager().getFallDistance() > 0f && canAIFall() || target != null && getBlock(0, 2, 0) != Blocks.AIR && getBlock(0, -1, 0) != Blocks.AIR)
