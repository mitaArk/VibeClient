    ...
    public void execute(LiteralArgumentBuilder<CommandSource> builder) {
        builder
                .then(literal("add")
                        .then(arg("player", word())
                                ...
                                .executes(context -> {
                                    ...
                                    if (!Vibe.getInstance().getFriendManager().isFriend(player)) {
                                        Vibe.getInstance().getFriendManager().add(player);
                ...
                .then(literal("remove")
                        .then(arg("player", word())
                                .suggests((context, builder1) -> {
                                    Vibe.getInstance().getFriendManager().getFriends().stream()
                                ...
                                .executes(context -> {
                                    String player = context.getArgument("player", String.class);
                                    if (Vibe.getInstance().getFriendManager().isFriend(player)) {
                                        Vibe.getInstance().getFriendManager().remove(player);
                ...
                .then(literal("list")
                        .executes(context -> {
                            ...
                            if (Vibe.getInstance().getFriendManager().getFriends().isEmpty()) ChatUtils.sendMessage(I18n.translate("commands.friend.empty"));
                            else {
                                for (int i = 0; i < Vibe.getInstance().getFriendManager().getFriends().size(); i++) {
                                    builder1.append(Vibe.getInstance().getFriendManager().getFriends().get(i));
                                    if (i < Vibe.getInstance().getFriendManager().getFriends().size() - 1) builder1.append(", ");
                                }
                                builder1.append(".");
                ...
                .then(literal("clear")
                        .executes(context -> {
                            if (!Vibe.getInstance().getFriendManager().isEmpty()) {
                                Vibe.getInstance().getFriendManager().clear();
