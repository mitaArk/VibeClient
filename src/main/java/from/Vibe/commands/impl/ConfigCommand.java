                ...
                .then(literal("save")
                        .then(arg("config", word())
                                .suggests((context, builder1) -> {
                                    if (Vibe.getInstance().getConfigsDir().exists() && Vibe.getInstance().getConfigsDir().isDirectory()) {
                                        File[] files = Vibe.getInstance().getConfigsDir().listFiles((dir, name) -> name.toLowerCase().endsWith(".drug"));
                                ...
                                .executes(context -> {
                                    ...
                                    try {
                                        Vibe.getInstance().getConfigManager().saveConfig(config);
                ...
                .then(literal("load")
                        .then(arg("config", word())
                                ...
                                .executes(context -> {
                                    String config = context.getArgument("config", String.class);
                                    try {
                                        Vibe.getInstance().getConfigManager().loadConfig(config);
