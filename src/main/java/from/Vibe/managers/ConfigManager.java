...
import from.Vibe.modules.impl.client.Panic;
import from.Vibe.modules.impl.client.UI;
import from.Vibe.modules.settings.Setting;
import from.Vibe.modules.settings.api.*;
import from.Vibe.modules.settings.impl.*;
import from.Vibe.utils.macro.Macro;
import from.Vibe.utils.other.FileUtils;
...
public class ConfigManager {
    ...
    public void saveConfig(String config) throws Exception {
        FileUtils.reset(Vibe.getInstance().getConfigsDir() + "/" + config + extension);
        JsonObject object = new JsonObject();
        object.add("config", new JsonPrimitive(config));
        object.add("modules", serializeModules());
        @Cleanup OutputStreamWriter writer = new OutputStreamWriter(new FileOutputStream(Vibe.getInstance().getConfigsDir() + "/" + config + extension), StandardCharsets.UTF_8);
    ...
    public void loadConfig(String config) throws Exception {
        if (!FileUtils.exists(Vibe.getInstance().getConfigsDir() + "/" + config + extension)) return;
        InputStream stream = Files.newInputStream(Paths.get(Vibe.getInstance().getConfigsDir() + "/" + config + extension));
    ...
    public void saveGlobals() throws Exception {
        FileUtils.reset(Vibe.getInstance().getGlobalsDir() + "/" + "globals" + extension);
        JsonObject object = new JsonObject();
        object.add("config", new JsonPrimitive(currentConfig));
        JsonArray friendsArray = new JsonArray();
        Vibe.getInstance().getFriendManager().getFriends().forEach(friendsArray::add);
        object.add("friends", friendsArray);
        JsonArray macrosArray = new JsonArray();
        Vibe.getInstance().getMacroManager().getMacros().forEach(macro -> macrosArray.add(macro.getName() + ":" + macro.getCommand() + ":" + macro.getBind().getKey()));
        object.add("macros", macrosArray);
        @Cleanup OutputStreamWriter writer = new OutputStreamWriter(new FileOutputStream(Vibe.getInstance().getGlobalsDir() + "/" + "globals" + extension), StandardCharsets.UTF_8);
        writer.write(new GsonBuilder().setPrettyPrinting().create().toJson(JsonParser.parseString(object.toString())));
    }

    public void loadGlobals() throws Exception {
        if (!FileUtils.exists(Vibe.getInstance().getGlobalsDir() + "/" + "globals" + extension)) return;
        InputStream stream = Files.newInputStream(Paths.get(Vibe.getInstance().getGlobalsDir() + "/" + "globals" + extension));
        JsonObject object = JsonParser.parseReader(new InputStreamReader(stream)).getAsJsonObject();
        ...
        if (object.has("friends")) {
            for (JsonElement element : object.get("friends").getAsJsonArray()) {
                if (Vibe.getInstance().getFriendManager().isFriend(element.getAsString())) continue;
                Vibe.getInstance().getFriendManager().add(element.getAsString());
            }
        }
        if (object.has("macros")) {
            for (JsonElement element : object.get("macros").getAsJsonArray()) {
                ...
                Vibe.getInstance().getMacroManager().add(new Macro(name, command, bind));
            }
        }
    }
