...
public class AutoVault extends Module {
    ...
    private void clickVault(BlockPos pos, BlockState state, VaultBlockEntity entity) {
        ...
        if (slot == -1) {
            mc.player.sendMessage(Text.of("[Vibe] Slot with ominous is empty"), false);
            return;
        }
        mc.player.sendMessage(Text.of("[Vibe] " + entity.getSharedData().getDisplayItem().getItem().getName().getString()), false);
