...
public class OffHand extends Module {
        ...
        if (needTotems()) {
            if (currentOffhandItem != Items.TOTEM_OF_UNDYING) {
                ...
                if (totemSlot != -1) {
                    ...
                    ticks = sync.getValue() ? Vibe.getInstance().getModuleManager().getModule(GuiMove.class).getTicks() : 0;
                }
            }
    ...
    private boolean needTotems() {
        ...
        if (fall.getValue() && Vibe.getInstance().getServerManager().getFallDistance() >= fallDistance.getValue()) return true;
