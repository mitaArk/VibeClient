...
import from.Vibe.modules.impl.movement.ElytraForward;
...
import from.Vibe.utils.animations.infinity.RotationAnimation;
import from.Vibe.utils.combat.IdealHitUtils;
import from.Vibe.utils.combat.PredictUtils;
import from.Vibe.utils.math.MathUtils;
import from.Vibe.utils.math.TimerUtils;
...
import from.Vibe.utils.world.MultipointUtils;
...
public class Aura extends Module {
    ...
    public void onPlayerTick(EventPlayerTick e) {
        ...
        if (target != null || !backTimer.passed(250)) {
        	...
        	if (target != null) {
                ...
                if (Vibe.getInstance().getModuleManager().getModule(ElytraForward.class).isToggled()) {
                    int predict = Vibe.getInstance().getModuleManager().getModule(ElytraForward.class).forward.getValue().intValue();
            ...
            if (target != null) {
                if (rotate.getValue() == Rotate.Packet) Vibe.getInstance().getRotationManager().addPacketRotation(currentRotations);
                else Vibe.getInstance().getRotationManager().addRotation(changer);
    ...
    private void attack() {
        ...
        if (sprint.getValue() == Sprint.Hvh
        		&& Vibe.getInstance().getServerManager().isServerSprinting()
