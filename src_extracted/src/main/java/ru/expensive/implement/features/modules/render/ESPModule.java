...
/**
 * ESP overlay: draws player name above head and shows armor + held items.
 * Rendering uses 2D overlay via DrawEvent and ProjectionUtil to project world to screen.
 * Depth test is disabled to make elements visible through walls.
 */
public class ESPModule extends Module implements QuickImports {
    ...
    public void onDraw(DrawEvent event) {
        ...
        // Draw through walls: disable depth test for overlay elements
        ...
        for (PlayerEntity player : mc.world.getPlayers()) {
            ...
            Vec3d headPos = new Vec3d(
                    player.getX(),
                    player.getY() + player.getHeight() + 0.3, // a bit above head
                    player.getZ()
            );

            var screen = ProjectionUtil.project(headPos.x, headPos.y, headPos.z);
            if (screen.x == Float.MAX_VALUE) continue; // not on screen or behind camera
            ...
            if (showNamesSetting.isValue()) {
                ...
                textYOffset += Fonts.getSize((int) nameSizeSetting.getValue()).getStringHeight(player.getName().getString()) + 2;
            }

            // Armor row under the name
            if (showArmorSetting.isValue()) {
                textYOffset += drawArmorRow(drawContext, stack, player, screenX, screenY + textYOffset);
            }

            // Held items row under armor
    ...
    private float drawArmorRow(DrawContext ctx, MatrixStack stack, PlayerEntity player, float centerX, float y) {
        ...
        return 16f + 2f; // row height + spacing
    }
